...
// Create a list of test cases.
        ...
        for (int i = 0; i < testCases.size(); i++) {
            ...
            System.out.println("\nTest Case " + (i + 1) + " Final Score: " +
                    score);
            overallScore += score;
            count++;
        }
        System.out.println("\n========================================");
        System.out.println("Average Score across all test cases: " +
                (overallScore / count));
    }
...
// Group 1: 4 students with full mutual roommate preferences.
        ...
        students.add(new UniversityStudent(
                ...
                Arrays.asList("Alice", "Charlie", "Frank"), Arrays.asList("Google",
                "Microsoft")
        ));
        students.add(new UniversityStudent(
...
// Two of them can be paired and one remains unpaired.
     ...
     * Accepts a list of UniversityStudent objects and a testCaseNumber (for output
     clarity).
     * Returns an integer score.
     */
    public static int gradeLab(List<UniversityStudent> students, int
            testCaseNumber) {
        int score = 0;
...
// Verify that each edge is reciprocal.
            for (UniversityStudent s : graph.getAllNodes()) {
                ...
                for (StudentGraph.Edge edge : edges) {
                    ...
                    for (StudentGraph.Edge reverseEdge :
                            graph.getNeighbors(neighbor)) {
                        if (reverseEdge.neighbor.equals(s) && reverseEdge.weight ==
                                edge.weight) {
                    ...
                    if (!reciprocalFound) {
                        throw new Exception("Graph edge from " + s.name + " to " +
                                neighbor.name + " is not reciprocal.");
                    }
                }
            }
            graph.displayGraph();
...
// in odd-sized groups, at most one can remain unpaired.
            ...
            if (unpairedCount > 1) {
                throw new Exception("Too many unpaired students: " +
                        unpairedCount);
            }
            score += 20;
...
// For test cases that don't have that internship, the returned pathmay be empty.
